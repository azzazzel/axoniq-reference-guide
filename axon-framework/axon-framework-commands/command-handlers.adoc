...
The annotation may be placed on either the field or an accessor method (e.g.
a getter) in the Command object.
...
If neither annotation works for your use case, a different `RoutingStrategy` can be configured, as is explained in the link:infrastructure.md#routing-strategy[Routing Strategy] section.
____
...
Thexref:../testing/commands-events.adoc[Aggregate Test Fixture] will guard from unintentional state changes in Command Handling functions.
...
Do note that in the scenario where Event Messages are published from an Event Sourcing Handler, the Event of the inner `apply()` invocation is only published to the entities after all entities have received the first event.
...
To make an Aggregate react on events from other Aggregate instances, link:../sagas/[Sagas] or xref:../events/event-handlers.adoc[Event Handling Components] should be leveraged
____
...
Command handling functions are most often directly placed on the Aggregate (as described in more detail link:command-handlers.md#aggregate-command-handlers[here]).
