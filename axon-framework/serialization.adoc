...
{% endtab %} {% tab title="Spring Boot AutoConfiguration - Configuration class" %}
...
For more information on how to configure your``EventSerializer`` to something different, check out the documentation about xref:./serialization.adoc[Serializers].
...
{% tabs %} {% tab title="[.code]``XStreamSerializer``" %}
...
{% tab title="[.code]``JacksonSerializer``" %}
...
An link:events/event-versioning.md#event-upcasting[upcaster] works on a given content type (e.g.
dom4j Document).
